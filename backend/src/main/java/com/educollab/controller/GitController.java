...
public class GitController {
    ...
    @PostMapping("/repositories/init/{projectId}") public void init(@PathVariable Long projectId) { gitService.ensureRepository(workspaceService.requireVisible(projectId, SecurityUtils.principal())); }
    ...
    @PostMapping("/branches") public void createBranch(@RequestBody BranchCreateRequest request) { workspaceService.requireVisible(request.projectId(), SecurityUtils.principal()); gitService.createBranch(request.projectId(), request.name()); }
    @GetMapping("/projects/{projectId}/commits")
    public java.util.List<CommitRecord> commits(@PathVariable Long projectId) {
    ...
    @GetMapping("/projects/{projectId}/tree") public java.util.List<GitService.TreeEntry> tree(@PathVariable Long projectId, @RequestParam(value = "path", required = false) String path) { workspaceService.requireVisible(projectId, SecurityUtils.principal()); return gitService.listTree(projectId, path); }
    @GetMapping("/projects/{projectId}/blob") public GitService.BlobView blob(@PathVariable Long projectId, @RequestParam("path") String path) { workspaceService.requireVisible(projectId, SecurityUtils.principal()); return gitService.readBlob(projectId, path); }
    @PostMapping("/merge-requests") public MergeRequestRecord createMr(@RequestBody MergeRequestSaveRequest request) { var project = workspaceService.requireVisible(request.projectId(), SecurityUtils.principal()); var mr = gitService.createMergeRequest(project, request.title(), request.sourceBranch(), request.targetBranch()); return new MergeRequestRecord(mr.getId(), mr.getTitle(), mr.getSourceBranch(), mr.getTargetBranch(), mr.getStatus().name()); }
    ...
    @PostMapping("/releases") public ReleaseRecord createRelease(@RequestBody ReleaseSaveRequest request) { var project = workspaceService.requireVisible(request.projectId(), SecurityUtils.principal()); var release = gitService.createRelease(project, request.version(), request.title(), request.description()); return new ReleaseRecord(release.getId(), release.getVersion(), release.getTitle(), release.getDescription(), release.getCreatedAt().toString()); }
    ...
    public GitCloneInfo cloneInfo(@PathVariable Long projectId) {
        ...
        return new GitCloneInfo(repo.getSlug(), httpUrl, "main");
    }
