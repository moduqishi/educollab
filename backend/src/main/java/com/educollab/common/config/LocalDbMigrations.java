...
public class LocalDbMigrations implements ApplicationRunner {
  ...
  public void run(ApplicationArguments args) {
    ...
    // documents: kind/office_ext/file_asset_id (markdown/office)
    ensureColumn("documents", "kind", "VARCHAR(20) NOT NULL DEFAULT 'MARKDOWN'");
    ...
    jdbc.execute("UPDATE documents SET kind='MARKDOWN' WHERE kind IS NULL");
    jdbc.execute("UPDATE documents SET kind='MARKDOWN' WHERE kind='NOTE'");
