...
export interface ServerOptions {
  ...
  /**
   * Called when this local "server" wants to broadcast a message to other peers.
   * The payload should be the exact object to be delivered to other clients as:
   * `socket.server.emit("message", payload)`.
   */
  broadcastMessage?: (payload: any) => void | Promise<void>;
  /**
   * Called when the editor triggers a "downloadas/save" flow and we have assembled
   * the final exported Office file (docx/xlsx/pptx).
   *
   * Returning/rejecting controls whether OnlyOffice will consider this save ok.
   */
  onSaveOfficeFile?: (args: {
    file: Blob;
    fileName: string;
    fileExt: string;
    cmd: any;
