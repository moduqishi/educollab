...
export interface MockSocketOptions {
  ...
  debug?: boolean;
  /**
   * Hook after client emits an event (client -> server direction).
   * Useful to bridge socket traffic to an external transport (e.g. Yjs room).
   */
  onClientEmit?: (event: string, args: any[]) => void;
  /**
   * Hook after server emits an event (server -> client direction).
   * Useful for debugging/telemetry.
   */
  onServerEmit?: (event: string, args: any[]) => void;
}
...
> {
  ...
  private _hooks: Pick<MockSocketOptions, "onClientEmit" | "onServerEmit">;

  constructor(options: MockSocketOptions = {}) {
    this._debug = options.debug ?? !!(import.meta as any).env?.DEV;
    this._hooks = { onClientEmit: options.onClientEmit, onServerEmit: options.onServerEmit };
  ...
  emit<E extends keyof EmitEvents & string>(
    ...
    const processEmit = async () => {
      ...
      try {
        this._hooks.onClientEmit?.(event, args as any[]);
      } catch {
        // ignore hook errors
      }
    };
  ...
  public server = {
    ...
    emit: (event: string, ...args: any[]) => {
      try {
        this._hooks.onServerEmit?.(event, args);
      } catch {
        // ignore hook errors
      }
      this._clientEmitter.emit(event, ...args);
