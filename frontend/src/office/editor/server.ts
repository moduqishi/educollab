...
export class EditorServer {
  ...
  /**
   * Set the "current user" for this local server instance.
   * This affects locks and the participant entry reported to the editor.
   */
  setUser(user: User) {
    this.user = user;
    // keep a stable sessionId per user to avoid editor glitches
    this.sessionId = `sess-${user.id}`;
  }

  /**
   * Force the document key (OnlyOffice `document.key` / internal URL suffix).
   * For EduCollab we bind this to `doc.collabKey` so multiple devices join the same room.
   */
  setKey(key: string) {
    this.id = key;
  }
  ...
  /**
   * Open a document from raw bytes (backend download).
   */
  async openBuffer(
    buffer: ArrayBuffer,
    opts: { fileType: string; title: string; key?: string },
  ) {
    this.fileType = opts.fileType;
    this.title = opts.title;
    this.id = opts.key || this.id || randomId();
    const documentType = getDocumentType(this.fileType);
    this.loadPromise = this.loadDocument(buffer, this.fileType);
    return { id: this.id, documentType };
  }
  ...
  async handleMessage(msg: any, ...args: unknown[]) {
    ...
    switch (msg.type) {
      ...
      case "getLock":
        ...
        send(releaseLockMsg);
        try {
          void this.options.broadcastMessage?.(getLockMsg);
          void this.options.broadcastMessage?.(releaseLockMsg);
        } catch {
          // ignore
        }
        break;
  ...
  async handleRequest(req: Request) {
    ...
    if (u.pathname.endsWith("/downloadas/" + key)) {
      ...
      const saveToBackend = async () => {
        ...
        const fileName = cmd.title || `Document.${this.fileType}`;
        const fileExt = (fileName.split(".").pop() || this.fileType).toLowerCase();

        try {
          if (this.options.onSaveOfficeFile) {
            await this.options.onSaveOfficeFile({
              file: blob,
              fileName,
              fileExt,
              cmd,
          ...
          console.error("[EditorServer] onSaveOfficeFile failed:", err);
          return { status: "error" };
        }
